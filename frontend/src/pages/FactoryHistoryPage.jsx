...
export function FactoryHistoryPage() {
  ...
  async function search() {
    if (!factoryName.trim()) { setError('请输入工厂名称'); return; }
    setError('');
    setLoading(true);
    try {
      const res = await api(`/api/history/factory?factoryName=${encodeURIComponent(factoryName.trim())}`);
      setData(res);
  ...
  return (
    <div>
      <div className="filter-row" style={{ marginBottom: 12 }}>
        <label>工厂名称：<input value={factoryName} onChange={(e) => setFactoryName(e.target.value)}
      ...
      {data && (
        <div>
          <h3>工厂：{data.factoryName}（共 {data.data.length} 个订单）</h3>
          {data.data.map((o) => (
          ...
          {data.data.length === 0 && <div style={{ color: '#888' }}>该工厂暂无订单记录</div>}
        </div>
      )}
    </div>
  );
